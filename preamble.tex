...
\usepackage{physics} % знаки модуля и нормы
...
% Подсчет числа таблиц и рисунков
...
\usepackage{rotating}
\newcommand{\addimgrotated}[4]{ % Добавить рисунок повернутым на 90 градусов 
        \begin{sidewaysfigure}
           \centering
            %\includegraphics[width=1.0\textwidth]{Image.eps}
            \includegraphics[height=#2\textheight, width=0.75\textwidth]{#1}
            \caption{#3}
            \label{#4}
        \end{sidewaysfigure}
}
...
% Секции без номеров (введение, заключение...), вместо subsection*{}
